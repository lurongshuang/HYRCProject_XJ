...
import com.hyrc99.projectByL.utils.view.LoadBaseDialog;
...
public abstract class BaseActivity extends AppCompatActivity {
     ...
     * 设置搜索状态栏
     *
     * @param isShowLeftRes 是否显示左侧
     * @param name          搜索 hint名称
     * @param onSearch      搜索事件
     * @param isHaveFocus   是否获得焦点弹出键盘
     */
    public void setSearchTilte(boolean isShowLeftRes, String name, SearchOnclickListener onSearch, boolean isHaveFocus) {
        Handler handler = null;
        EditText etSearch = findViewById(R.id.etSearch_search);
        if (etSearch != null) {
            SpannableString ss = new SpannableString(name);
            AbsoluteSizeSpan ass = new AbsoluteSizeSpan((int) getResources().getDimension(R.dimen.sp_14), false);
            ss.setSpan(ass, 0, ss.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            etSearch.setHint(new SpannedString(ss));
        }
        FontIconView fontIconView = findViewById(R.id.iv_leftIcon_search);
        if (fontIconView != null) {
            fontIconView.setVisibility(isShowLeftRes ? View.VISIBLE : View.GONE);
            fontIconView.setOnClickListener(new View.OnClickListener() {
                @Override
        ...
        TextView tv_rightText = findViewById(R.id.tv_rightText_search);
        if (tv_rightText != null) {
            tv_rightText.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    hideKeyboard(view);
                    RelativeLayout tp = (RelativeLayout) view.getParent();
                    if (tp != null) {
                        EditText editText = tp.findViewById(R.id.etSearch_search);
                        if (editText != null) {
            ...
            etSearch.setOnEditorActionListener(new TextView.OnEditorActionListener() {
                ...
                public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
                    if (actionId == EditorInfo.IME_ACTION_SEARCH) {
                        hideKeyboard(v);
                        onSearch.OnSearch(v.getText().toString());
                        return true;
                    }
                    return false;
                }
